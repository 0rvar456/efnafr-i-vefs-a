[efnafræði.html](https://github.com/user-attachments/files/25391429/efnafraedi.html)
<!DOCTYPE html>
<html lang="is">
<head>
<meta charset="UTF-8" />
<meta name="viewport" content="width=device-width, initial-scale=1.0" />
<title>Efnablanda · Lotukerfi · Hvarf · Prófílar</title>
...
<header>
  <div class="logo">
    ...
    <div class="logo-text">
      <h1>Efnablanda</h1>
      <p>Lotukerfi · Prófil · Hvarf</p>
    </div>
  </div>
  <div class="header-right">
    Avogadro-talan: <span>6.02214076 × 10²³ mol⁻¹</span><br>
    n = m/M &nbsp;·&nbsp; N = n × Nₐ &nbsp;·&nbsp; m = n × M
  </div>
</header>
...
<div class="main-layout">
  ...
  <div class="table-section">
    <div class="sec-title">Lotukerfið — smelltu til að bæta í blöndu · hægri smellur velur “frumefni-prófíl”</div>
    <div class="legend">
      <div class="leg"><div class="leg-dot" style="background:rgba(239,68,68,.8)"></div>Leðurjárn</div>
      <div class="leg"><div class="leg-dot" style="background:rgba(251,146,60,.8)"></div>Jarðleðurjárn</div>
      <div class="leg"><div class="leg-dot" style="background:rgba(56,189,248,.8)"></div>Umskiptiefni</div>
      <div class="leg"><div class="leg-dot" style="background:rgba(148,163,184,.8)"></div>Málmar</div>
      <div class="leg"><div class="leg-dot" style="background:rgba(34,211,238,.8)"></div>Hálfmálmar</div>
      <div class="leg"><div class="leg-dot" style="background:rgba(0,229,160,.8)"></div>Óstefni</div>
      <div class="leg"><div class="leg-dot" style="background:rgba(167,139,250,.8)"></div>Halógen</div>
      <div class="leg"><div class="leg-dot" style="background:rgba(251,191,36,.8)"></div>Göfgar lofttegundir</div>
    </div>

    <div class="info-bar" id="infoBar">
      <span style="font-size:1.7rem;opacity:.18;">⚛</span>
      <span style="color:var(--dim);">Sveipaðu yfir frumefni til að sjá upplýsingar...</span>
    </div>
  ...
  <div class="right-panel">
    ...
    <div class="formula-box">
      ...
      <span class="formula-text" id="formulaText">—</span>
      <span class="mmass-pill" id="mmassPill" style="display:none">— g/mol</span>
    </div>

    <!-- ELEMENT PROFILE (FASTUR KAFI) -->
    <div class="box">
      <div class="box-title">Frumefni — sjá aðeins um eitt</div>
      <div class="row">
        <select id="profileSel" onchange="setSelected(this.value)"><option value="">Veldu frumefni</option></select>
        <button class="btn btn-blue" onclick="setSelectedFromMix()">Velja úr blöndu</button>
        <button class="btn" onclick="clearSelected()">Hreinsa val</button>
      </div>
      <div class="out" id="profileOut">Veldu frumefni til að sjá prófíl.</div>
    </div>

    <!-- REACTION (2 ELEMENTS) -->
    <div class="box">
      <div class="box-title">Veldu 2 — geta þau efnahvarfast?</div>
      <div class="row">
      ...
      <div class="out" id="rxOut">Veldu tvö frumefni til að fá niðurstöðu.</div>
    </div>

    <!-- COMPOUND FINDER (2+ ELEMENTS) -->
    <div class="box">
      <div class="box-title">Veldu 2+ — hvaða efni/sameind getur myndast?</div>
      <div class="row" style="width:100%">
        <select id="cmpSel" multiple size="6" onchange="updateCompoundFinder()" style="min-width:100%"></select>
      </div>
      <div class="out" id="cmpOut">Veldu 2 eða fleiri (CTRL/⌘ fyrir mörg).</div>
      <div class="subtle" style="margin-top:.55rem;font-size:.72rem;">
        (Leitar í algengum efnum + reiknar tvíefna jónasölt sjálfkrafa ef við á.)
      </div>
    </div>
    ...
    <div class="mix-list" id="mixList">
      <div class="empty-state" id="emptyState">
        <div class="big">🧪</div>
        Smelltu á frumefni í lotukerfinu<br>
        til að bæta þeim við blönduna.<br>
        Settu inn <strong style="color:var(--g)">grömm</strong>, <strong style="color:var(--b)">mól</strong> eða <strong style="color:var(--v)">atóm</strong>.
      </div>
    </div>

    <!-- TOTALS -->
    <div class="totals-section" id="totalsSection" style="display:none;">
      <div class="sec-title" style="margin-bottom:.7rem;">Samtölur</div>
      <div class="totals-grid">
        <div class="tc tc-mol">
          <div class="tc-lbl">Heildar Mól</div>
          <div class="tc-val" id="ttlMol">—</div>
          <div class="tc-unit">mol</div>
        </div>
        <div class="tc tc-mass">
          <div class="tc-lbl">Heildar Massi</div>
          <div class="tc-val" id="ttlMass">—</div>
          <div class="tc-unit">g</div>
        </div>
        <div class="tc tc-mm">
          <div class="tc-lbl">Meðal Mólmassi</div>
          <div class="tc-val" id="ttlMmass">—</div>
          <div class="tc-unit">g/mol</div>
        </div>
        <div class="tc tc-avo">
          <div class="tc-lbl">Heildar Eindir</div>
          <div class="tc-val" id="ttlAvo">—</div>
          <div class="tc-unit">× 10²³</div>
        </div>
      </div>
    </div>

    <div class="formulas-footer">
      <div class="sec-title" style="margin-bottom:.5rem;">Formúlur</div>
      <div class="fml">
        <div class="fml-row"><span class="fml-eq">n = m ÷ M</span><span>n=mól, m=massi(g), M=mólmassi(g/mol)</span></div>
        <div class="fml-row"><span class="fml-eq">N = n × Nₐ</span><span>N=fjöldi einda, Nₐ=6.022×10²³</span></div>
        <div class="fml-row"><span class="fml-eq">m = n × M</span><span>massi = mól × mólmassi</span></div>
      </div>
    </div>
...
const EL = [
  ...
  {n:2,s:'He',name:'Helín',m:4.0026,c:'noble',g:18,p:1},
  {n:3,s:'Li',name:'Litín',m:6.94,c:'alkali',g:1,p:2},
  {n:4,s:'Be',name:'Beryllín',m:9.0122,c:'alkali-earth',g:2,p:2},
  {n:5,s:'B',name:'Bór',m:10.81,c:'metalloid',g:13,p:2},
  {n:6,s:'C',name:'Kolefni',m:12.011,c:'nonmetal',g:14,p:2},
  {n:7,s:'N',name:'Nitur',m:14.007,c:'nonmetal',g:15,p:2},
  {n:8,s:'O',name:'Súrefni',m:15.999,c:'nonmetal',g:16,p:2},
  {n:9,s:'F',name:'Flúor',m:18.998,c:'halogen',g:17,p:2},
  {n:10,s:'Ne',name:'Neon',m:20.180,c:'noble',g:18,p:2},
  {n:11,s:'Na',name:'Natríum',m:22.990,c:'alkali',g:1,p:3},
  {n:12,s:'Mg',name:'Magnesín',m:24.305,c:'alkali-earth',g:2,p:3},
  {n:13,s:'Al',name:'Ál',m:26.982,c:'post-trans',g:13,p:3},
  {n:14,s:'Si',name:'Kísill',m:28.085,c:'metalloid',g:14,p:3},
  {n:15,s:'P',name:'Fosfór',m:30.974,c:'nonmetal',g:15,p:3},
  {n:16,s:'S',name:'Brennisteinn',m:32.06,c:'nonmetal',g:16,p:3},
  {n:17,s:'Cl',name:'Klór',m:35.45,c:'halogen',g:17,p:3},
  {n:18,s:'Ar',name:'Argon',m:39.948,c:'noble',g:18,p:3},
  {n:19,s:'K',name:'Kalín',m:39.098,c:'alkali',g:1,p:4},
  {n:20,s:'Ca',name:'Kalsín',m:40.078,c:'alkali-earth',g:2,p:4},
  {n:21,s:'Sc',name:'Skandín',m:44.956,c:'transition',g:3,p:4},
  {n:22,s:'Ti',name:'Títan',m:47.867,c:'transition',g:4,p:4},
  {n:23,s:'V',name:'Vanadín',m:50.942,c:'transition',g:5,p:4},
  {n:24,s:'Cr',name:'Króm',m:51.996,c:'transition',g:6,p:4},
  {n:25,s:'Mn',name:'Mangan',m:54.938,c:'transition',g:7,p:4},
  {n:26,s:'Fe',name:'Járn',m:55.845,c:'transition',g:8,p:4},
  {n:27,s:'Co',name:'Kóbalt',m:58.933,c:'transition',g:9,p:4},
  ...
  {n:31,s:'Ga',name:'Gallín',m:69.723,c:'post-trans',g:13,p:4},
  {n:32,s:'Ge',name:'Germanín',m:72.630,c:'metalloid',g:14,p:4},
  {n:33,s:'As',name:'Arsen',m:74.922,c:'metalloid',g:15,p:4},
  {n:34,s:'Se',name:'Selen',m:78.971,c:'nonmetal',g:16,p:4},
  {n:35,s:'Br',name:'Bróm',m:79.904,c:'halogen',g:17,p:4},
  {n:36,s:'Kr',name:'Kryptón',m:83.798,c:'noble',g:18,p:4},
  {n:37,s:'Rb',name:'Rúbidín',m:85.468,c:'alkali',g:1,p:5},
  {n:38,s:'Sr',name:'Strontín',m:87.62,c:'alkali-earth',g:2,p:5},
  {n:39,s:'Y',name:'Ytrín',m:88.906,c:'transition',g:3,p:5},
  {n:40,s:'Zr',name:'Sirkónín',m:91.224,c:'transition',g:4,p:5},
  {n:41,s:'Nb',name:'Nióbín',m:92.906,c:'transition',g:5,p:5},
  {n:42,s:'Mo',name:'Mólýbden',m:95.95,c:'transition',g:6,p:5},
  {n:43,s:'Tc',name:'Teknetín',m:98,c:'transition',g:7,p:5},
  {n:44,s:'Ru',name:'Rútenín',m:101.07,c:'transition',g:8,p:5},
  {n:45,s:'Rh',name:'Ródín',m:102.91,c:'transition',g:9,p:5},
  {n:46,s:'Pd',name:'Palladín',m:106.42,c:'transition',g:10,p:5},
  {n:47,s:'Ag',name:'Silfur',m:107.87,c:'transition',g:11,p:5},
  {n:48,s:'Cd',name:'Kadmín',m:112.41,c:'transition',g:12,p:5},
  {n:49,s:'In',name:'Indín',m:114.82,c:'post-trans',g:13,p:5},
  {n:50,s:'Sn',name:'Tin',m:118.71,c:'post-trans',g:14,p:5},
  {n:51,s:'Sb',name:'Antimón',m:121.76,c:'metalloid',g:15,p:5},
  {n:52,s:'Te',name:'Tellúr',m:127.60,c:'metalloid',g:16,p:5},
  {n:53,s:'I',name:'Joð',m:126.90,c:'halogen',g:17,p:5},
  {n:54,s:'Xe',name:'Xenon',m:131.29,c:'noble',g:18,p:5},
  {n:55,s:'Cs',name:'Sesín',m:132.91,c:'alkali',g:1,p:6},
  {n:56,s:'Ba',name:'Barín',m:137.33,c:'alkali-earth',g:2,p:6},

  {n:57,s:'La',name:'Lantan',m:138.91,c:'lanthanide',g:3,p:8},
  {n:58,s:'Ce',name:'Cer',m:140.12,c:'lanthanide',g:4,p:8},
  {n:59,s:'Pr',name:'Praseódým',m:140.91,c:'lanthanide',g:5,p:8},
  {n:60,s:'Nd',name:'Neódým',m:144.24,c:'lanthanide',g:6,p:8},
  {n:61,s:'Pm',name:'Prómetín',m:145,c:'lanthanide',g:7,p:8},
  {n:62,s:'Sm',name:'Samarín',m:150.36,c:'lanthanide',g:8,p:8},
  {n:63,s:'Eu',name:'Evrópín',m:151.96,c:'lanthanide',g:9,p:8},
  {n:64,s:'Gd',name:'Gadólíníum',m:157.25,c:'lanthanide',g:10,p:8},
  {n:65,s:'Tb',name:'Terbíum',m:158.93,c:'lanthanide',g:11,p:8},
  {n:66,s:'Dy',name:'Dysprósíum',m:162.50,c:'lanthanide',g:12,p:8},
  {n:67,s:'Ho',name:'Hólmíum',m:164.93,c:'lanthanide',g:13,p:8},
  {n:68,s:'Er',name:'Erbíum',m:167.26,c:'lanthanide',g:14,p:8},
  {n:69,s:'Tm',name:'Túlíum',m:168.93,c:'lanthanide',g:15,p:8},
  {n:70,s:'Yb',name:'Ytterbíum',m:173.04,c:'lanthanide',g:16,p:8},
  {n:71,s:'Lu',name:'Lútesíum',m:174.97,c:'lanthanide',g:17,p:8},

  {n:72,s:'Hf',name:'Hafníum',m:178.49,c:'transition',g:4,p:6},
  {n:73,s:'Ta',name:'Tantal',m:180.95,c:'transition',g:5,p:6},
  {n:74,s:'W',name:'Wolfram',m:183.84,c:'transition',g:6,p:6},
  {n:75,s:'Re',name:'Rheníum',m:186.21,c:'transition',g:7,p:6},
  {n:76,s:'Os',name:'Ósmíum',m:190.23,c:'transition',g:8,p:6},
  {n:77,s:'Ir',name:'Irídíum',m:192.22,c:'transition',g:9,p:6},
  {n:78,s:'Pt',name:'Platína',m:195.08,c:'transition',g:10,p:6},
  {n:79,s:'Au',name:'Gull',m:196.97,c:'transition',g:11,p:6},
  {n:80,s:'Hg',name:'Kvikasilfur',m:200.59,c:'transition',g:12,p:6},
  {n:81,s:'Tl',name:'Tallíum',m:204.38,c:'post-trans',g:13,p:6},
  {n:82,s:'Pb',name:'Blý',m:207.2,c:'post-trans',g:14,p:6},
  {n:83,s:'Bi',name:'Bismút',m:208.98,c:'post-trans',g:15,p:6},
  {n:84,s:'Po',name:'Pólóníum',m:209,c:'metalloid',g:16,p:6},
  {n:85,s:'At',name:'Astatín',m:210,c:'halogen',g:17,p:6},
  {n:86,s:'Rn',name:'Radon',m:222,c:'noble',g:18,p:6},
  {n:87,s:'Fr',name:'Fransíum',m:223,c:'alkali',g:1,p:7},
  {n:88,s:'Ra',name:'Radíum',m:226,c:'alkali-earth',g:2,p:7},

  {n:89,s:'Ac',name:'Aktíníum',m:227,c:'actinide',g:3,p:9},
  {n:90,s:'Th',name:'Þóríum',m:232.04,c:'actinide',g:4,p:9},
  {n:91,s:'Pa',name:'Prótaktíníum',m:231.04,c:'actinide',g:5,p:9},
  {n:92,s:'U',name:'Úran',m:238.03,c:'actinide',g:6,p:9},
  {n:93,s:'Np',name:'Neptúníum',m:237,c:'actinide',g:7,p:9},
  {n:94,s:'Pu',name:'Plútóníum',m:244,c:'actinide',g:8,p:9},
  {n:95,s:'Am',name:'Ameríkíum',m:243,c:'actinide',g:9,p:9},
  {n:96,s:'Cm',name:'Kúríum',m:247,c:'actinide',g:10,p:9},
  {n:97,s:'Bk',name:'Berkelíum',m:247,c:'actinide',g:11,p:9},
  {n:98,s:'Cf',name:'Kaliforníum',m:251,c:'actinide',g:12,p:9},
  {n:99,s:'Es',name:'Einsteíníum',m:252,c:'actinide',g:13,p:9},
  {n:100,s:'Fm',name:'Fermíum',m:257,c:'actinide',g:14,p:9},
  {n:101,s:'Md',name:'Mendelévíum',m:258,c:'actinide',g:15,p:9},
  {n:102,s:'No',name:'Nóbelíum',m:259,c:'actinide',g:16,p:9},
  {n:103,s:'Lr',name:'Lawrensíum',m:262,c:'actinide',g:17,p:9},

  {n:104,s:'Rf',name:'Rutherfordíum',m:267,c:'transition',g:4,p:7},
  {n:105,s:'Db',name:'Dubníum',m:268,c:'transition',g:5,p:7},
  {n:106,s:'Sg',name:'Seaborgíum',m:271,c:'transition',g:6,p:7},
  {n:107,s:'Bh',name:'Bohrium',m:270,c:'transition',g:7,p:7},
  {n:108,s:'Hs',name:'Hassíum',m:277,c:'transition',g:8,p:7},
  {n:109,s:'Mt',name:'Meitneríum',m:276,c:'transition',g:9,p:7},
  {n:110,s:'Ds',name:'Darmstadtíum',m:281,c:'transition',g:10,p:7},
  {n:111,s:'Rg',name:'Röntgeníum',m:282,c:'transition',g:11,p:7},
  {n:112,s:'Cn',name:'Kópernikíum',m:285,c:'transition',g:12,p:7},
  {n:113,s:'Nh',name:'Nihóníum',m:286,c:'post-trans',g:13,p:7},
  {n:114,s:'Fl',name:'Fleróvíum',m:289,c:'post-trans',g:14,p:7},
  {n:115,s:'Mc',name:'Moskóvíum',m:290,c:'post-trans',g:15,p:7},
  {n:116,s:'Lv',name:'Livermóríum',m:293,c:'post-trans',g:16,p:7},
  {n:117,s:'Ts',name:'Tennessín',m:294,c:'halogen',g:17,p:7},
  {n:118,s:'Og',name:'Óganessón',m:294,c:'noble',g:18,p:7},
];

/* ===================== COMMON COMPOUNDS LIST ===================== */
const KNOWN = [
  {formula:"H2O", name:"Vatn"},
  {formula:"H2O2", name:"Vetnisperoxíð"},
  {formula:"CO2", name:"Koltvísýringur"},
  {formula:"CO", name:"Kolmónoxíð"},
  {formula:"CH4", name:"Metan"},
  {formula:"NH3", name:"Ammoníak"},
  {formula:"NaCl", name:"Borðsalt (natríumklóríð)"},
  {formula:"KCl", name:"Kalíumklóríð"},
  {formula:"MgCl2", name:"Magnesíumklóríð"},
  {formula:"CaCl2", name:"Kalsíumklóríð"},
  {formula:"NaOH", name:"Natríumhýdroxíð"},
  {formula:"HCl", name:"Vetnisklóríð (saltsýra í vatni)"},
  {formula:"H2SO4", name:"Brennisteinssýra"},
  {formula:"HNO3", name:"Saltpéturssýra"},
  {formula:"NaHCO3", name:"Matarsódi (natríumvetniskarbónat)"},
  {formula:"CaCO3", name:"Kalsíumkarbónat (kalksteinn/kríta)"},
  {formula:"Fe2O3", name:"Járn(III)oxíð (ryð)"},
  {formula:"Al2O3", name:"Áloxíð"},
  {formula:"MgO", name:"Magnesíumoxíð"},
  {formula:"SO2", name:"Brennisteinsdíoxíð"},
  {formula:"SO3", name:"Brennisteinstríoxíð"},
  {formula:"NO2", name:"Niturdíoxíð"},
  {formula:"O2", name:"Súrefni (sameind)"},
  {formula:"N2", name:"Nitur (sameind)"},
  {formula:"H2", name:"Vetni (sameind)"},
  {formula:"Cl2", name:"Klór (sameind)"},
];
...
function labelCat(c){
  const map = {
    'alkali':'Leðurjárn (alkali)',
    'alkali-earth':'Jarðleðurjárn',
    'transition':'Umskiptiefni',
    'post-trans':'Málmur',
    'metalloid':'Hálfmálmur',
    'nonmetal':'Óstefni',
    'halogen':'Halógen',
    'noble':'Göfug lofttegund',
    'lanthanide':'Lantaníð',
    'actinide':'Aktaníð'
  };
  return map[c] || c;
...
function reactivityMiniText(el){
  if(isNoble(el)) return 'Göfug lofttegund: ytri rafeindaskel er oft full → hvarfast sjaldan.';
  if(el.c==='alkali') return 'Leðurjárn: 1 gildisrafeind → missir hana auðveldlega → mjög hvarfgjarnt.';
  if(el.c==='halogen') return 'Halógen: vantar oft 1 rafeind → tekur hana auðveldlega → mjög hvarfgjarnt (með málmum).';
  if(isMetal(el)) return 'Málmur: vill oft missa rafeindir → + jón → hvarfast gjarnan við óstefni.';
  if(isMetalloid(el)) return 'Hálfmálmur: milliheima → hvarfgirni fer oft eftir pari og aðstæðum.';
  return 'Óstefni: deilir oft rafeindum (samgilt) eða tekur rafeindir til að verða stöðugra.';
}
...
function predictReactionAndProduct(a,b){
  if(isNoble(a) || isNoble(b)){
    ...
    return {
      likely:false,
      title:"Ólíklegt (göfug lofttegund)",
      explain:`${ng.s} er göfug lofttegund og hvarfast venjulega ekki við venjulegar aðstæður.`,
      product:null,
      also:[]
    };
  }
  ...
  if((isMetal(a) && isNonmetal(b)) || (isMetal(b) && isNonmetal(a))){
    ...
    if(ionic){
      ...
      product = {
        ...
        name: known ? known.name : "Jónasamband (salt)"
      };
    }
    return {
      likely:true,
      title:"Mjög líklegt (jónatengi)",
      explain:`Málmur missir rafeindir (+ jón) og óstefni tekur rafeindir (− jón). Jónir dragast saman → salt.`,
      product,
      also:[]
    };
  }
  ...
  if(isNonmetal(a) && isNonmetal(b)){
    ...
    return {
      likely:true,
      title:"Líklegt (samgilt tengi)",
      explain:`Óstefni + óstefni deila oft rafeindum. Hvaða sameind myndast fer eftir hlutföllum/stuðlum.`,
      product:null,
      also
    };
  }
  ...
  if(isMetal(a) && isMetal(b)){
    return {
      likely:false,
      title:"Ólíklegt sem “sameind”",
      explain:`Málmur + málmur myndar sjaldan skýra sameind (oft málmblendi eða engin einföld formúla).`,
      product:null,
      also:[]
    };
  }
  ...
  if(isMetalloid(a) || isMetalloid(b)){
    ...
    return {
      likely:true,
      title:"Stundum (fer eftir aðstæðum)",
      explain:`Hálfmálmar eru milliheima. Útkoma fer oft eftir hlutföllum, hitastigi og hvaða efni hitt er.`,
      product:null,
      also
    };
  }
  ...
  return {
    likely:false,
    title:"Óvíst",
    explain:"Þetta par fellur ekki skýrt í einföldu reglurnar hér.",
    product:null,
    also:[]
  };
}
...
function showInfo(el,color){
  ...
  b.innerHTML = `
    ...
    <div class="info-details">
      ...
      <div class="info-meta">Sætistala: ${el.n} · Flokkur: <strong style="color:${color}">${labelCat(el.c)}</strong> · Mólmassi: <strong style="color:${color}">${el.m} g/mol</strong></div>
    </div>
    <div class="info-hint">${mix[el.s] ? '✓ Í blöndu — smelltu til að fjarlægja' : 'Smelltu til að bæta við'}<br><span style="opacity:.7">Hægri smellur = prófíl</span></div>
  `;
}
function clearInfo(){
  const b = document.getElementById('infoBar');
  b.className = 'info-bar';
  b.innerHTML = `<span style="font-size:1.7rem;opacity:.18;">⚛</span><span style="color:var(--dim);">Sveipaðu yfir frumefni til að sjá upplýsingar...</span>`;
}
...
function removeEl(s){
  ...
  if(c) c.classList.remove('active');
  render();
  // if removed is selected in profile, keep profile but show “not in mix”
  refreshProfile();
}
...
function refreshProfile(){
  ...
  if(!selectedSym){
    out.textContent = "Veldu frumefni til að sjá prófíl.\nTip: hægri smelltu á efni í lotukerfinu til að velja það.";
    return;
  }
...
- eindir: ≈ ${fmtAvo(r.particles)} ×10²³`;
  } else {
    inMixTxt = "\n\n(Þetta frumefni er ekki í blöndunni.)";
  }
...
Hópur: ${el.g} · Tímabil: ${el.p}
Mólmassi: ${el.m} g/mol

Af hverju hvarfast það?
...
function updateReaction(){
  ...
  if(!aSym || !bSym){
    out.textContent = "Veldu tvö frumefni til að fá niðurstöðu.";
    return;
  }
  if(aSym === bSym){
    out.textContent = "Veldu tvö mismunandi frumefni.";
    return;
  }
  ...
  let productLine = "";
  if(res.product){
    productLine = `\n\nMyndar líklega: ${res.product.pretty} = ${res.product.name}`;
  } else if(res.also && res.also.length){
    const list = res.also.slice(0,8).map(k => `${toSubscriptFormula(k.formula)} = ${k.name}`).join("\n- ");
    productLine = `\n\nÞekkt efni með þessum tveimur (fer eftir hlutföllum):\n- ${list}`;
  } else {
    productLine = `\n\nEngin “beint” vara fannst í listanum. (Getur samt hvarfast við ákveðnar aðstæður.)`;
  }

  out.textContent =
`${a.name} (${a.s}) + ${b.name} (${b.s})

Niðurstaða: ${res.likely ? "✅" : "❌"} ${res.title}
...
function updateCompoundFinder(){
  ...
  if(chosen.length < 2){
    out.textContent = "Veldu 2 eða fleiri (CTRL/⌘ fyrir mörg).";
    return;
  }
  ...
  if(chosen.length === 2){
    ...
    if(a && b && ((isMetal(a) && isNonmetal(b)) || (isMetal(b) && isNonmetal(a)))){
      ...
      if(ionic){
        const known = KNOWN.find(k => k.formula === ionic.formula);
        ionicLine = `\n\nSjálfvirkt jónasamband:\n- ${toSubscriptFormula(ionic.formula)} = ${known ? known.name : "Jónasamband (salt)"}`;
      }
    }
  }

  let lines = [];
  lines.push(`Valið: ${chosen.join(", ")}`);
  lines.push("");

  if(matches.length){
    lines.push("Þekkt efni/sameindir sem passa við valið:");
    matches.slice(0,14).forEach(k=>{
      lines.push(`- ${toSubscriptFormula(k.formula)} = ${k.name}`);
    });
  } else {
    lines.push("Engin þekkt sameind fannst í listanum sem passar við þessi frumefni.");
  }
...
function render(){
  ...
  keys.forEach(s=>{
    ...
    const unitLbl = entry.mode==='g'?'g': entry.mode==='mol'?'mol':'atóm';
      ...
      <div>
        ...
        <div class="input-row">
          ...
          <div class="mode-tabs">
            ...
            <button class="mtab ${atOn}" onclick="setMode('${s}','atom')">atóm</button>
          </div>
          <button class="mini-btn" onclick="setSelected('${s}')">Prófíl</button>
        </div>
        <div class="derived" id="d-${s}">
          <span class="dv">${fmt(r.mol,4)}</span> mol &nbsp;·&nbsp;
          <span class="dv">${fmt(r.mass,4)}</span> g &nbsp;·&nbsp;
          <span class="da"><span class="dv">${fmtAvo(r.particles)}</span> ×10²³</span>
        </div>
      </div>
      <div style="display:flex;flex-direction:column;align-items:flex-end;gap:.3rem">
        <button class="remove-x" onclick="removeEl('${s}')" title="Fjarlægja">×</button>
      </div>
    `;
...
function updateTotals(mol,mass,part,n){
  ...
  document.getElementById('ttlMmass').textContent = (n>0 && mol>0) ? fmt(mass/mol,4) : '—';
...
function fillSelectors(){
  ...
  prof.innerHTML = `<option value="">Veldu frumefni</option>` + EL.slice().sort((a,b)=>a.n-b.n)
    .map(e=>`<option value="${e.s}">${e.s} — ${e.name}</option>`).join('');
  ...
  cmp.innerHTML = EL.slice().sort((a,b)=>a.n-b.n)
    .map(e=>{
      ...
      return `<option value="${e.s}" ${sel}>${e.s} — ${e.name}</option>`;
    }).join('');
}
